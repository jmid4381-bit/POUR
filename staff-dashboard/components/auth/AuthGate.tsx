...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  // password, not to land straight in the dashboard with their old one gone.
  if (recovering) {
    ...
    return (
      <div className="min-h-screen bg-void flex flex-col items-center justify-center gap-4 px-4 text-center">
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-raised border border-border text-slate-400 hover:text-white hover:border-rim transition-colors text-sm font-body"
        >
          Sign out
        </button>
      </div>
